...
if __name__ == '__main__':
    ...
    # 此次使用在中文领域效果较优的chinese-xlnet-mid模型
    MODEL_NAME = "chinese-xlnet-mid"
    model = ppnlp.transformers.XLNetForSequenceClassification.from_pretrained(MODEL_NAME, num_classes=14) # 此次分类任务为14分类任务，故num_classes设置为14
    # 定义模型对应的tokenizer，tokenizer可以把原始输入文本转化成模型model可接受的输入数据格式。需注意tokenizer类要与选择的模型相对应，具体可以查看PaddleNLP相关文档
    tokenizer = ppnlp.transformers.XLNetTokenizer.from_pretrained(MODEL_NAME)
